...
calculate_performance_score() {
    ...
    if [ -n "$CPU_MPSTAT_OUTPUT" ]; then
        ...
        log_file "DEBUG: CPU_MPSTAT_OUTPUT idle_percent: '$idle_percent'"
        if [ -n "$idle_percent" ]; then
            local normalized_idle=$(normalize_score "$idle_percent" 10 90 0)
            log_file "DEBUG: normalized_idle: '$normalized_idle'"
            cpu_idle_score="$normalized_idle"
        fi
    fi
    ...
    if [ -n "$CPU_TOP_OUTPUT" ]; then
        ...
        log_file "DEBUG: CPU_TOP_OUTPUT load_avg_1min: '$load_avg_1min', num_cores: '$num_cores'"
        if [ -n "$load_avg_1min" ] && [ -n "$num_cores" ] && [ "$num_cores" -gt 0 ]; then
            ...
            log_file "DEBUG: max_load: '$max_load', min_load: '$min_load'"
            local normalized_load=$(normalize_score "$load_avg_1min" "$min_load" "$max_load" 1)
            log_file "DEBUG: normalized_load: '$normalized_load'"
            cpu_load_score="$normalized_load"
        fi
    fi
    ...
    if [ -n "$MEM_MEMINFO_OUTPUT" ]; then
        ...
        log_file "DEBUG: MEM_MEMINFO_OUTPUT mem_total_kb: '$mem_total_kb', mem_free_kb: '$mem_free_kb'"
        if [ -n "$mem_total_kb" ] && [ -n "$mem_free_kb" ] && [ "$mem_total_kb" -gt 0 ]; then
            local free_ratio=$(echo "scale=4; $mem_free_kb / $mem_total_kb" | bc -l)
            log_file "DEBUG: free_ratio: '$free_ratio'"
            local normalized_free=$(normalize_score "$(echo "scale=2; $free_ratio * 100" | bc -l)" 10 80 0)
            log_file "DEBUG: normalized_free: '$normalized_free'"
            mem_free_score="$normalized_free"
        fi
        ...
        log_file "DEBUG: MEM_MEMINFO_OUTPUT swap_total_kb: '$swap_total_kb', swap_free_kb: '$swap_free_kb'"
        if [ -n "$swap_total_kb" ] && [ -n "$swap_free_kb" ] && [ "$swap_total_kb" -gt 0 ]; then
            ...
            log_file "DEBUG: swap_used_kb: '$swap_used_kb', swap_used_ratio: '$swap_used_ratio'"
            local normalized_swap=$(normalize_score "$(echo "scale=2; $swap_used_ratio * 100" | bc -l)" 0 10 1)
            log_file "DEBUG: normalized_swap: '$normalized_swap'"
            swap_used_score="$normalized_swap"
    ...
    if [ -n "$DISK_IOSTAT_OUTPUT" ]; then
        ...
        log_file "DEBUG: DISK_IOSTAT_OUTPUT avg_util: '$avg_util'"
        if [ -n "$avg_util" ]; then
            local normalized_disk=$(normalize_score "$avg_util" 5 50 1)
            log_file "DEBUG: normalized_disk: '$normalized_disk'"
            disk_util_score="$normalized_disk"
        fi
    fi
    ...
    if [ -n "$NET_PING_OUTPUT" ]; then
        ...
        log_file "DEBUG: NET_PING_OUTPUT avg_latency: '$avg_latency', packet_loss: '$packet_loss'"

        if [ -n "$avg_latency" ]; then
            local normalized_latency=$(normalize_score "$avg_latency" 20 200 1)
            log_file "DEBUG: normalized_latency: '$normalized_latency'"
            net_latency_score="$normalized_latency"
        fi
        if [ -n "$packet_loss" ]; then
            local normalized_packet_loss=$(normalize_score "$packet_loss" 0 10 1)
            log_file "DEBUG: normalized_packet_loss: '$normalized_packet_loss'"
            net_packet_loss_score="$normalized_packet_loss"
        fi
    fi

    log_file "\n--- End Debugging Score Calculation ---"
