...
#
# ┌────────────────────────────────────────────────────────────────────────────┐
# │ PyAnalyzer v3.8 :: Single-File Python Codebase AST Analyzer                │
# ├────────────────────────────────────────────────────────────────────────────┤
# │ I/O SCHEMA                                                                 │
# │   ├─ CLI (Input):  $ ./pyanalyzer.sh [OPTIONS] [TARGET_DIRECTORY]           │
# │   └─ OUT (Result): STDOUT (Colored Tree) + File (___analysis___.log)        │
# │                                                                            │
# │ FETCHED JSON STRUCTURE (.analysis_results."path/to/file.py")               │
# │   └─ {                                                                     │
# │        "imports": [ "module1", ... ],                                      │
# │        "details": {                                                        │
# │          "classes": [ { "type": "class", "name": "...", "doc": "...",      │
# │                         "methods": [ { (function_struct) } ] } ],           │
# │          "functions": [ {                                                  │
# │             (function_struct):                                             │
# │               "type": "function", "name": "my_func", "doc": "...",         │
# │               "args": [ { "name": "a", "type_hint": "int", "default": 0}], │
# │               "returns": { "type_hint": "str" },                           │
# │               "body_analysis": [                                           │
# │                  { "type": "global_decl", "name": "..." },                 │
# │                  { "type": "var_decl", "name": "v", "var_type": "T" },     │
# │                  { "type": "ext_call", "call": "ext.func" } ]              │
# │          } ]                                                               │
# │        }                                                                   │
# │      }                                                                     │
# └────────────────────────────────────────────────────────────────────────────┘
#
# ==============================================================================
# PyAnalyzer v3.8 - Single-File Python Codebase Analysis Tool
#
...
# By default, it saves a clean, un-colored report to ___analysis___.log.
# ==============================================================================
...
readonly REQUIRED_TOOLS=(python3 jq find grep sed)
readonly OUTPUT_LOG_FILE="___analysis___.log"
...
readonly VENV_PATTERNS=("*venv*" "env" ".env" "__pypackages__")
readonly DEFAULT_EXCLUDE_FILES=("*.pyc" "*.pyo" "*.pyd" "*.so" ".*.swp")
readonly DEFAULT_INCLUDE_FORMATS=("*.py")
readonly C_BOLD=$'\033[1m' C_CYAN=$'\033[1;36m' C_GREEN=$'\033[1;32m' C_YELLOW=$'\033[1;33m' C_MAGENTA=$'\033[0;35m' C_BLUE=$'\033[0;36m' C_GRAY=$'\033[0;90m' C_DIM=$'\033[2m' C_NC=$'\033[0m'
...
info_nn() { log_nn 3 "$1"; }
debug() { log 4 "$1"; }
strip_colors() { sed -E "s/\x1B\[[0-9;]*[mK]//g"; }
format_debug_command() { local str="Effective find command:"; for arg in "$@"; do if [[ "$arg" == -* || "$arg" == "(" || "$arg" == ")" ]]; then str+=" ${C_DIM}${arg}${C_NC}"; else str+=" ${C_CYAN}'${arg}'${C_NC}"; fi; done; debug "$str"; }
declare -A METRICS
...
build_find_command() {
    ...
    if [[ "$INCLUDE_ALL_FILES" == "false" ]]; then
        ...
        if [[ "$INCLUDE_VENV" == "false" ]]; then exclude_dirs+=("${VENV_PATTERNS[@]}"); fi
    ...
    if [[ "$DEBUG_MODE" == "true" ]]; then format_debug_command "${FIND_COMMAND_ARGS[@]}"; fi
}
...
def get_source_segment(source_lines, node):
    try: return ast.get_source_segment(source="\n".join(source_lines), node=node)
    except Exception: return None
def resolve_attribute_chain(node):
    if isinstance(node, ast.Name): return node.id
    if isinstance(node, ast.Attribute):
        base = resolve_attribute_chain(node.value)
...
def analyze_function(node, source_lines):
    name=node.name; docstring=ast.get_docstring(node) or ""; doc_details=parse_docstring(docstring); args=[]
    all_args=node.args.posonlyargs+node.args.args; defaults=node.args.defaults; arg_offset=len(all_args)-len(defaults)
    for i, arg in enumerate(all_args):
        arg_info={"name":arg.arg,"type_hint":"","default":None}
        if arg.annotation: arg_info["type_hint"]=get_source_segment(source_lines,arg.annotation) or ""
        if i>=arg_offset: default_node=defaults[i-arg_offset]; arg_info["default"]=get_source_segment(source_lines,default_node) or "..."
        args.append(arg_info)
    returns={"type_hint":""};
    if node.returns: returns["type_hint"]=get_source_segment(source_lines,node.returns) or ""
    body_analysis = []
    seen = set()
    for sub_node in ast.walk(node):
        item = None
        if isinstance(sub_node, ast.Global):
            item = ("global_decl", ", ".join(sub_node.names))
            body_analysis.append({"type": "global_decl", "name": ", ".join(sub_node.names)})
        elif isinstance(sub_node, ast.AnnAssign) and isinstance(sub_node.target, ast.Name):
            type_hint = get_source_segment(source_lines, sub_node.annotation) or ""
            item = ("var_decl", sub_node.target.id)
            body_analysis.append({"type": "var_decl", "name": sub_node.target.id, "var_type": type_hint})
        elif isinstance(sub_node, ast.Call):
            call_name = resolve_attribute_chain(sub_node.func)
            if call_name and "." in call_name:
                item = ("ext_call", call_name)
                body_analysis.append({"type": "ext_call", "call": call_name})
        if item and item not in seen:
            seen.add(item)
    unique_body = [dict(t) for t in {tuple(d.items()) for d in body_analysis}]
    return {"type":"function","name":name,"doc":doc_details["purpose"],"args":args,"returns":returns,"body_analysis":unique_body}
def analyze_file(filepath,base_dir,source_lines):
    tree=ast.parse("\n".join(source_lines),filename=str(filepath)); imports,details=set(),{"classes":[],"functions":[]}
    ...
    for node in tree.body:
        if isinstance(node,ast.FunctionDef): details["functions"].append(analyze_function(node,source_lines))
        elif isinstance(node,ast.ClassDef):
            doc=ast.get_docstring(node) or ""
            class_info={"type":"class","name":node.name,"doc":parse_docstring(doc)["purpose"],"methods":[]}
            for sub_node in node.body:
                if isinstance(sub_node,ast.FunctionDef): class_info["methods"].append(analyze_function(sub_node,source_lines))
            details["classes"].append(class_info)
    return {"imports":sorted(list(imports)),"details":details}
try:
    base_dir=Path(sys.argv[1]).resolve(); all_files_data={}; filepaths_str=sys.argv[2:]
    for fp_str in filepaths_str:
        fp=Path(fp_str); relative_path=str(fp.relative_to(base_dir)); print(f"FILE:{relative_path}",file=sys.stderr,flush=True)
        try:
            with open(fp,"r",encoding="utf-8",errors="ignore") as f: source_lines=f.read().splitlines()
            all_files_data[relative_path]=analyze_file(fp,base_dir,source_lines)
        except Exception as e: print(f"ERROR:Could not process {relative_path}: {e}",file=sys.stderr,flush=True); continue
    print(json.dumps(all_files_data,indent=2))
except Exception as e: print(f"FATAL:{e}",file=sys.stderr,flush=True); print("{}")
...
render_text_report() {
    ...
    echo -e "${C_DIM}Legend: ${C_CYAN}C${C_DIM} Class | ${C_GREEN}F${C_DIM}/${C_GREEN}M${C_DIM} Func/Method | ${C_YELLOW}→${C_DIM} Imports/Returns | ${C_GRAY}G${C_DIM} Global | ${C_GRAY}V${C_DIM} Var | ${C_GRAY}↳${C_DIM} Call${C_NC}"
    echo "$json_report" | jq -r --arg C_BOLD "$C_BOLD" --arg C_CYAN "$C_CYAN" --arg C_GREEN "$C_GREEN" --arg C_YELLOW "$C_YELLOW" --arg C_GRAY "$C_GRAY" --arg C_DIM "$C_DIM" --arg C_NC "$C_NC" '
    .analysis_results | to_entries[] | .key as $path | .value as $file_data | (
        "\n▫︎ " + $C_BOLD + $C_YELLOW + $path + $C_NC +
        ([$file_data.details.functions[]? | . as $func | "\n  " + $C_GREEN + "F " + $func.name + "(" + ($func.args | map(.name + (if .type_hint != "" then ":" + .type_hint else "" end) + (if .default then "=" + .default else "" end)) | join(", ")) + ")" + $C_NC + (if $func.doc and $func.doc != "" then "\n    " + $C_DIM + $func.doc + $C_NC else "" end) + (if $func.returns.type_hint and $func.returns.type_hint != "None" and $func.returns.type_hint != "" then "\n    " + $C_YELLOW + "→ Returns: " + $C_NC + $C_DIM + $func.returns.type_hint + $C_NC else "" end) +
        ([$func.body_analysis[]? | . as $detail |
            if .type == "global_decl" then "\n    " + $C_GRAY + "G Global: " + .name + $C_NC
            elif .type == "var_decl" then "\n    " + $C_GRAY + "V Var: " + .name + (if .var_type and .var_type != "" then ": " + .var_type else "" end) + $C_NC
            elif .type == "ext_call" then "\n    " + $C_GRAY + "↳ Call: " + .call + $C_NC
            else "" end
        ] | join(""))
        ] | join("")) +
        ([$file_data.details.classes[]? | . as $class | "\n  " + $C_CYAN + "C " + $class.name + $C_NC + (if $class.doc and $class.doc != "" then "\n    " + $C_DIM + $class.doc + $C_NC else "" end) +
        ([$class.methods[]? | . as $method | "\n    " + $C_GREEN + "M " + $method.name + "(" + ($method.args | map(.name + (if .type_hint != "" then ":" + .type_hint else "" end) + (if .default then "=" + .default else "" end)) | join(", ")) + ")" + $C_NC + (if $method.doc and $method.doc != "" then "\n      " + $C_DIM + $method.doc + $C_NC else "" end) + (if $method.returns.type_hint and $method.returns.type_hint != "None" and $method.returns.type_hint != "" then "\n      " + $C_YELLOW + "→ Returns: " + $C_NC + $C_DIM + $method.returns.type_hint + $C_NC else "" end) +
        ([$method.body_analysis[]? | . as $detail |
            if .type == "global_decl" then "\n      " + $C_GRAY + "G Global: " + .name + $C_NC
            elif .type == "var_decl" then "\n      " + $C_GRAY + "V Var: " + .name + (if .var_type and .var_type != "" then ": " + .var_type else "" end) + $C_NC
            elif .type == "ext_call" then "\n      " + $C_GRAY + "↳ Call: " + .call + $C_NC
            else "" end
...
analyze_project() {
    ...
    local tmp_json tmp_entries progress_pipe
    tmp_json=$(mktemp)
    tmp_entries=$(mktemp)
    progress_pipe=$(mktemp -u)
    mkfifo "$progress_pipe"
    trap 'rm -f -- "$tmp_json" "$tmp_entries" "$progress_pipe"' EXIT SIGHUP SIGINT SIGTERM
    ...
    echo -ne "\033[?25l"; exec 3< "$progress_pipe"
    ...
    echo "$entry_points" | sed '/^\s*$/d' | jq -R . | jq -s . > "$tmp_entries"

    local final_json; final_json=$(jq --argfile ent "$tmp_entries" --arg v "$VERSION" --arg d "$TARGET_DIR" \
    '{ "metadata": { "tool_version": $v, "analysis_timestamp": (now|todate), "target_directory": $d }, "entry_points": $ent, "analysis_results": . }' < "$tmp_json")

    rm -f -- "$tmp_json" "$tmp_entries" "$progress_pipe"; trap - EXIT SIGHUP SIGINT SIGTERM
    if [[ -z "$final_json" ]]; then error "Failed to consolidate the final JSON report."; return 1; fi

    # --- Final Output Handling ---
    local final_report
    case "$OUTPUT_FORMAT" in
        json) final_report=$(echo "$final_json" | jq --color-output .) ;;
        text|tree) final_report=$(echo "$final_json" | render_text_report) ;;
        *) error "Unknown output format: '$OUTPUT_FORMAT'"; return 1 ;;
    esac

    if [[ "$SAVE_TO_FILE" == "true" ]]; then
        # Send colored output to terminal, and stripped output to file
        echo -e "$final_report" | tee >(strip_colors > "$OUTPUT_LOG_FILE")
        info "Report also saved to: ${C_GREEN}${OUTPUT_LOG_FILE}${C_NC}"
    else
        echo -e "$final_report"
    fi
}
...
By default, a clean report is saved to ___analysis___.log and a colored
...
OPTIONS:
  ...
  --no-output-file           Prevent saving the report to a file.
  -d, --dry-run              Perform checks but do not run the full analysis.
  -D, --debug                Enable verbose debug logging.
  -h, --help                 Display this help message and exit.
  -v, --version              Display script version and exit.
DEPENDENCIES: bash (v4+), python3, jq, find, grep, sed
EOF
}
parse_arguments() { while [[ $# -gt 0 ]]; do case "$1" in -f|--format) OUTPUT_FORMAT="$2"; shift 2 ;; --include-formats) IFS=',' read -r -a ADDITIONAL_FORMATS <<< "$2"; shift 2 ;; --include-all-files) INCLUDE_ALL_FILES=true; shift ;; --venv) INCLUDE_VENV=true; shift ;; --no-output-file) SAVE_TO_FILE=false; shift ;; -d|--dry-run) DRY_RUN=true; shift ;; -D|--debug) DEBUG_MODE=true; LOG_LEVEL=4; shift ;; -h|--help) show_help; exit 0 ;; -v|--version) echo "$SCRIPT_NAME v$VERSION"; exit 0 ;; --) shift; break ;; -*) error "Unknown option: $1"; show_help; exit 1 ;; *) TARGET_DIR="$1"; shift ;; esac; done; }
main() {
    ...
    build_find_command
    if [[ "$DRY_RUN" == "true" ]]; then info "Dry-run mode enabled."; exit 0; fi
